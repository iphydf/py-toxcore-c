...
cdef class Tox_Ptr:
    ...
    def conference_get_peer_count(self, conference_number: Tox_Conference_Number) -> int:
        ...
        cdef int count = tox_conference_peer_count(self._get(), conference_number, &err)
        if err:
    ...
    def conference_get_offline_peer_count(self, conference_number: Tox_Conference_Number) -> int:
        cdef Tox_Err_Conference_Peer_Query err = TOX_ERR_CONFERENCE_PEER_QUERY_OK
        cdef int count = tox_conference_offline_peer_count(self._get(), conference_number, &err)
        if err:
            raise ApiException(Tox_Err_Conference_Peer_Query(err))
        return count
    ...
    def conference_offline_peer_get_public_key(self, conference_number: Tox_Conference_Number, peer_number: Tox_Conference_Offline_Peer_Number) -> bytes:
        cdef Tox_Err_Conference_Peer_Query err = TOX_ERR_CONFERENCE_PEER_QUERY_OK
        cdef size_t size = tox_public_key_size()
        cdef uint8_t *data = <uint8_t*> malloc(size * sizeof(uint8_t))
        try:
            tox_conference_offline_peer_get_public_key(self._get(), conference_number, peer_number, data, &err)
            if err:
                raise ApiException(Tox_Err_Conference_Peer_Query(err))
            return data[:size]
        finally:
    ...
    def group_get_topic(self, group_number: Tox_Group_Number) -> bytes:
        cdef Tox_Err_Group_State_Queries err = TOX_ERR_GROUP_STATE_QUERIES_OK
        cdef size_t size = tox_group_get_topic_size(self._get(), group_number, &err)
        if err:
            raise ApiException(Tox_Err_Group_State_Queries(err))
        cdef uint8_t *data = <uint8_t*> malloc(size * sizeof(uint8_t))
        try:
    ...
    def group_set_password(self, group_number: Tox_Group_Number, password: bytes) -> None:
        cdef Tox_Err_Group_Founder_Set_Password err = TOX_ERR_GROUP_FOUNDER_SET_PASSWORD_OK
        # TODO(iphydf): Rename this function to tox_group_set_password.
        tox_group_founder_set_password(self._get(), group_number, password, len(password), &err)
        if err:
            raise ApiException(Tox_Err_Group_Founder_Set_Password(err))

    def group_mod_kick_peer(self, group_number: Tox_Group_Number, peer_id: Tox_Group_Peer_Number) -> None:
        cdef Tox_Err_Group_Mod_Kick_Peer err = TOX_ERR_GROUP_MOD_KICK_PEER_OK
        tox_group_mod_kick_peer(self._get(), group_number, peer_id, &err)
        if err:
            raise ApiException(Tox_Err_Group_Mod_Kick_Peer(err))
