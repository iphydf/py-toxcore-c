...
cdef extern from "tox/toxencryptsave.h":
    ...
    cdef bool tox_pass_encrypt(const uint8_t* plaintext, size_t plaintext_len, const uint8_t* passphrase, size_t passphrase_len, uint8_t* data, Tox_Err_Encryption* error)
    cdef bool tox_pass_decrypt(const uint8_t* ciphertext, size_t ciphertext_len, const uint8_t* passphrase, size_t passphrase_len, uint8_t* data, Tox_Err_Decryption* error)
    cdef void tox_pass_key_free(Tox_Pass_Key* self)
    ...
    cdef bool tox_pass_key_encrypt(const Tox_Pass_Key* self, const uint8_t* plaintext, size_t plaintext_len, uint8_t* data, Tox_Err_Encryption* error)
    cdef bool tox_pass_key_decrypt(const Tox_Pass_Key* self, const uint8_t* ciphertext, size_t ciphertext_len, uint8_t* data, Tox_Err_Decryption* error)
    cdef bool tox_get_salt(const uint8_t* ciphertext, uint8_t* data, Tox_Err_Get_Salt* error)
